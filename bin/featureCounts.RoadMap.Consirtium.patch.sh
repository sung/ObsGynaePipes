...
#for i in `ls ~/data/RoadMap/Consortium/RNA-Seq/Somatic/GSM*HuFGM01.bed.gz`; do
for i in `ls ~/data/RoadMap/Consortium/RNA-Seq/Somatic/GSM*Brain_Hippocampus_Middle.mRNA-Seq*.bed.gz`; do
	echo $i; 
	time bedtools coverage -a $i -b $GFF -s -counts | \
		...
		sed 's/"//g' > ${i%.bed.gz}.$MY_GENE.exon_id.cnt.txt; 
done
echo -e "ALL done\n"
