...
## output: snp.raw.vcf and non.cpg.raw.vcf
...
if [ $RUN_BS_GENO_BY_CHR -eq 1 ]; then
	...
	if [ -s ${My_bam%.bam}.RG.recal.bam ]; then
		...
		time java -Xmx$BIS_MEM -jar $BIS_SNP \
			...
			-C CHG,1 \
			-L $Chr \
			-out_modes $BIS_OUTPUT_MODE \
			-stand_call_conf $BIS_CALL_CONF \
			-stand_emit_conf $BIS_EMIT_CONF \
			-mmq $BIS_MMQ \
			-mbq $BIS_MBQ \
			-vfn1 $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.vcf
...
if [ $RUN_BS_SORT_BY_CHR -eq 1 ]; then
	...
	if [ -s $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.vcf ]; then
		echo -e "perl $BIS_SNP_SORTER PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.vcf -tmp $PROJECT_DIR/scratch/$Barcode\n"
		time perl $BIS_SNP_SORTER \
			--k 1 --c 2 \
			--tmp $PROJECT_DIR/scratch/$Barcode \
			$PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.vcf \
			$GENOME_DIR/hg19b.fa.fai > $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.sorted.vcf
	else
...
##################################################
## 6. Filter Fake SNPs
## output: non.cpg.filtered.vcf
## VCFpostprocess does not support "-nt"
##################################################
if [ $RUN_BS_FILTER_BY_CHR -eq 1 ]; then
	if [ -s $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.sorted.vcf ]; then
		echo -e "java -Xmx$BIS_MEM -jar $BIS_SNP -T VCFpostprocess -oldVcf $Barcode.$Chr.non.cpg.raw.vcf"
		time java -Xmx$BIS_MEM -jar $BIS_SNP \
			-T VCFpostprocess \
			-R $REF_SEQ \
			-oldVcf $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.sorted.vcf \
			-newVcf $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.filtered.vcf \
			-snpVcf $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.snp.raw.sorted.vcf \
			-o $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.filter.summary.txt

		#if [ -s $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.filtered.vcf ]; then
		#	echo -e "rm $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.sorted.vcf \n"
		#	time rm $PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.sorted.vcf 
		#fi
	else
		echo -e "$PROJECT_DIR/BisSNP/$Barcode/$Chr/$SLX.$Barcode.$Chr.non.cpg.raw.sorted.vcf not found"
		exit
	fi 
