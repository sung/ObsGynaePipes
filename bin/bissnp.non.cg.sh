...
## -out_modes (or --output_modes)
## DEFAULT_FOR_TCGA: emit all of CpG sites above emit threshold into vcf1 file, and all of SNP sites above
## emit threshold into vcf2 file
## EMIT_VARIANT_AND_CYTOSINES: emit all of Cytosine sites above emit threshold into vcf1 file, and all of SNP sites above 
## emit threshold into vcf2 file.
##############################################
VFN1=$PROJECT_DIR/BisSNP/$Barcode/$SLX.$Barcode.non.cpg.raw.vcf
...
if [ $RUN_BS_GENO -eq 1 ]; then
	...
	# SLX-8074.SLX-8080.A001.r_1_val_1.fq.gz_bismark_bt2_pe.q1.deduplicated.sorted.RG.recal.nkx1.2.top3.bam
	if [ -s $My_bam ]; then
		echo -e "java -Xmx$BIS_MEM -jar $BIS_SNP -T BisulfiteGenotyper -I $My_bam"
		if [ -n "$TARGET" ]; then
			time java -Xmx$BIS_MEM -jar $BIS_SNP \
...
if [ $RUN_BS_FILTER -eq 1 ]; then
	if [ -s ${VFN2%.vcf}.sorted.vcf ]; then
		echo -e "java -Xmx$BIS_MEM -jar $BIS_SNP -T VCFpostprocess -oldVcf $Barcode.snp.raw.vcf"
		if [ -s $TARGET ]; then
			time java -Xmx$BIS_MEM -jar $BIS_SNP \
				-T VCFpostprocess \
				-R $REF_SEQ \
	...
	if [ -s ${VFN1%.vcf}.sorted.vcf ]; then
		echo -e "java -Xmx$BIS_MEM -jar $BIS_SNP -T VCFpostprocess -oldVcf $Barcode.cpg.raw.vcf"
		if [ -s $TARGET ]; then
			time java -Xmx$BIS_MEM -jar $BIS_SNP \
				-T VCFpostprocess \
				-R $REF_SEQ \
