...
if [ $IS_PE -eq 1 ]; then
	...
	if [ $RUN_TRIM -eq 1 ]; then
		...
		My_bam=$PROJECT_DIR/Bismark/Alignment/$Barcode/$Barcode.r_1_val_1.fq.gz_bismark_bt2_pe.bam # i'm expecting this at mapping stage
	else
		My_bam=$PROJECT_DIR/Bismark/Alignment/$Barcode/$Barcode.r_1.fq.gz_bismark_bt2_pe.bam # i'm expecting this at mapping stage
	fi # end of run trim
	...
	##################################################### 
	# 5. Remove duplicate reads and poorly mapped reads #
	# [todo] is this step necessary? any alternative tool? (e.g. picard)
	# time: (NT=20 for A001)
	# input: .bam
	...
	if [ $RUN_BM_DEDUP -eq 1 ]; then
		if [ -s $My_bam ]; then
			echo deduplicate_bismark $My_bam
		...
		# expecting A011.r_1_val_1.fq.gz_bismark_bt2_pe.deduplicated.bam 
		if [ -s ${My_bam%.bam}.deduplicated.bam ]; then
			echo -e "samtools view -u -F 0x100 ${My_bam%.bam}.deduplicated.bam | samtools flagstat - > ${My_bam%.bam}.deduplicated.bam.flagstat"
			time samtools view -u -F 0x100 ${My_bam%.bam}.deduplicated.bam | samtools flagstat - > ${My_bam%.bam}.deduplicated.bam.flagstat
			...
			# output: A011.r_1_val_1.fq.gz_bismark_bt2_pe.deduplicated.q20.bam
			echo -e "samtools view -bh -q $MAPQ ${My_bam%.bam}.deduplicated.bam -o ${My_bam%.bam}.deduplicated.q$MAPQ.bam "
			time samtools view -bh -q $MAPQ ${My_bam%.bam}.deduplicated.bam -o ${My_bam%.bam}.deduplicated.q$MAPQ.bam 
		...
		# expecting  A011.r_1_val_1.fq.gz_bismark_bt2_pe.deduplicated.q20.bam
		if [ -s ${My_bam%.bam}.deduplicated.q$MAPQ.bam ]; then
			echo -e "samtools view -u -F 0x100 ${My_bam%.bam}.deduplicated.q$MAPQ.bam | samtools flagstat - > ${My_bam%.bam}.deduplicated.q$MAPQ.bam.flagstat"
			time samtools view -u -F 0x100 ${My_bam%.bam}.deduplicated.q$MAPQ.bam | samtools flagstat - > ${My_bam%.bam}.deduplicated.q$MAPQ.bam.flagstat
		else
			echo -e "\e[031m$My_bam not found\e[0m\n"
			exit
		fi
	...
	if [ $RUN_MERGE_BAM -eq 1 ]; then
		if [ -s ${My_bam%.bam}.deduplicated.q$MAPQ.bam ] && [ -s $Other_bam ]; then
		...
		My_bam=${My_bam%.bam}.deduplicated.q$MAPQ.bam # e.g. A011.r_1_val_1.fq.gz_bismark_bt2_pe.deduplicated.q20.bam
	fi
