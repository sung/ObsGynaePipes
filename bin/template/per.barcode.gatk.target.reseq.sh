...
if [ $RUN_GATK_BASE_RECAL -eq 1 ]; then
	...
	if [ -s ${My_bam%.bam}.recalibrated.bam ]; then
		if [ -s ${My_bam%.bam}.recalibrated.bai ]; then
			...
			echo -e "samtools index -@ $NT ${My_bam%.bam}.recalibrated.bam\n"
			time samtools index -@ $NT ${My_bam%.bam}.recalibrated.bam 
		fi
	else
...
#	echo -e "samtools index -@ $NT ${My_bam%.bam}.OnTarget.q$MAPQ.bam\n"
#	time samtools index -@ $NT ${My_bam%.bam}.OnTarget.q$MAPQ.bam
#fi
#
...
## via HaplotypeCaller ##
#########################
My_HC_VCF=$PROJECT_DIR/GATK/$Barcode/$SLX.$Barcode.$Cell.bwa.recalibrated.HC.snp.indel.vcf.gz 
My_HC_Indel_VCF=$PROJECT_DIR/GATK/$Barcode/$SLX.$Barcode.$Cell.bwa.recalibrated.HC.indel.vcf.gz 
My_HC_SNP_VCF=$PROJECT_DIR/GATK/$Barcode/$SLX.$Barcode.$Cell.bwa.recalibrated.HC.snp.vcf.gz 
...
if [ $RUN_GATK_HC -eq 1 ];then
    My_bam=$PROJECT_DIR/BWA/$Barcode/$SLX.$Barcode.$Cell.bwa.samblaster.recalibrated.bam 
	if [ ! -s $My_bam ];then
		echo -e "\e[031m$My_bam is expected but not found\e[0m\n"
		exit
	fi
