...
if [ $IS_PE -eq 1 ]; then
	...
	#######################
	# remove orphan reads
	# output: $Barcode.rescued.fw.fq.gz $Barcode.rescued.rv.fq.gz
	#######################
	# only at butterfly
	if [ $RUN_RM_ORPHAN -eq 1 ]; then
		if [ -s $Merged_fw_FastQ ] && [ -s $Merged_rv_FastQ ]
		then
			echo "removing orphan reads..."
			time mkfifo /disk2/ssg29/$Barcode.fq1.tmp  #$PROJECT_DIR/Trim/$Barcode/tmp_fq1

			time zcat $Merged_fw_FastQ | awk 'NR%4==1{n=$1}NR%4==2{s=$1}NR%4==0{print n,s,$1}' | sort -S $BM_BUFFER_SIZE > /disk2/ssg29/$Barcode.fq1.tmp &

			time zcat $Merged_rv_FastQ | awk 'NR%4==1{n=$1}NR%4==2{s=$1}NR%4==0{print n,s,$1}' | sort -S $BM_BUFFER_SIZE | join /disk2/ssg29/$Barcode.fq1.tmp - | awk "NF==5{print \$1\"\n\"\$2\"\n+\n\"\$3 > \"/disk2/ssg29/$Barcode.rescued.fw.fq\";print \$1\"\n\"\$4\"\n+\n\"\$5 > \"/disk2/ssg29/$Barcode.rescued.rv.fq\"}"
		else
			echo -e "\e[031m$Merged_fw_FastQ or $Merged_rv_FastQ not found\e[0m\n"
			exit
		fi

		if [ -s /disk2/ssg29/$Barcode.rescued.fw.fq ]; then
			echo -e "gzip /disk2/ssg29/$Barcode.rescued.fw.fq"
			time gzip /disk2/ssg29/$Barcode.rescued.fw.fq
		else
			echo -e "\e[031m/disk2/ssg29/$Barcode.rescued.fw.fq not found\e[0m\n"
			exit
		fi

		if [ -s /disk2/ssg29/$Barcode.rescued.rv.fq ]; then
			echo -e "gzip /disk2/ssg29/$Barcode.rescued.fw.fq"
			time gzip /disk2/ssg29/$Barcode.rescued.rv.fq
			time mv /disk2/ssg29/$Barcode.rescued.* $PROJECT_DIR/Trim/$Barcode/
		else
			echo -e "\e[031m/disk2/ssg29/$Barcode.rescued.rv.fq not found\e[0m\n"
			exit
		fi

		time rm /disk2/ssg29/$Barcode.fq1.tmp
	fi
	...
	if [ $RUN_GENOMECOV -eq 1 ]; then
		if [ -s ${My_bam%.bam}.sorted.bam ]; then
			...
			time bedtools genomecov \
				-ibam ${My_bam%.bam}.sorted.bam > $PROJECT_DIR/Coverage/$Barcode/$Barcode.genomecov.txt

			# Depth of Coveage
			time grep -P '^genome' $PROJECT_DIR/Coverage/$Barcode/$Barcode.genomecov.txt | awk '{sum+=$2*$3} END { print "depth of coverage= ",sum/$genomeSize}' > $PROJECT_DIR/Coverage/$Barcode/$Barcode.coverage.txt

			# Breadh of Coverage
			time grep -P '^genome\s+[123456789]' $PROJECT_DIR/Coverage/$Barcode/$Barcode.genomecov.txt | awk '{sum+=$3} END {print "breadh of coveage= ", sum/$genomeSize}' >> $PROJECT_DIR/Coverage/$Barcode/$Barcode.coverage.txt		
		fi
	else
