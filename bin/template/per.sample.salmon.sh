...
# do 1)FastQC 2)trim 3)tophat 4)bedtool, 5)htseq 
	...
	# SALMON 
	###########
	SALMON_OUT=$PROJECT_DIR/Salmon/$Barcode
	if [ $RUN_SALMON -eq 1 ]; then
		mkdir_unless $PROJECT_DIR/Salmon
		mkdir_unless $SALMON_OUT
	...
	if [ -s $SALMON_OUT/quant.sf ]; then
		...
		#awk '/TCONS/{printf "%s\t%.f\n", $1,$5}' $SALMON_OUT/quant.sf > $SALMON_OUT/$SLX.$Barcode.quant.POPS.$TR_PREFIX.$ENS_VER.novel.10.tcons.clean.txt
	fi
	if [ -s $SALMON_OUT/quant.genes.sf ]; then
		...
		#awk '/XLOC/{printf "%s\t%.f\n", $1,$5}' $SALMON_OUT/quant.genes.sf > $SALMON_OUT/$SLX.$Barcode.quant.POPS.$TR_PREFIX.$ENS_VER.novel.10.xloc.clean.txt 
	fi
